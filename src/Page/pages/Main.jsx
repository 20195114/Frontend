...
const Main = () => {
  ...
  const fetchData = useCallback(async (url, key, user_id = null) => {
    ...
    try {
      const response = await axios.get(`${process.env.REACT_APP_EC2_ADDRESS}${url}${user_id ? `/${user_id}` : ''}`);
      console.log(`${key} response:`, response.data); // Debugging statement
      setState(prevState => ({ ...prevState, [key]: response.data }));
    } catch (error) {
      console.error(`Error fetching ${key}:`, error);
    } finally {
...
//   const fetchData = useCallback(async (url, key, user_id = null, method = 'get') => {
//     setLoading(prevState => ({ ...prevState, [key]: true }));
//     try {
//       const response = method === 'get'
//         ? await axios.get(`${process.env.REACT_APP_EC2_ADDRESS}${url}${user_id ? `/${user_id}` : ''}`)
//         : await axios.post(`${process.env.REACT_APP_CUD_ADDRESS}${url}${user_id ? `/${user_id}` : ''}`);

//       console.log(`${key} response:`, response.data); // Debugging statement
//       setState(prevState => ({ ...prevState, [key]: response.data.vods || response.data }));
//     } catch (error) {
//       console.error(`Error fetching ${key}:`, error);
//     } finally {
//       setLoading(prevState => ({ ...prevState, [key]: false }));
...
//     fetchData('/mainpage/home/youtube', 'youtubeTrendsVods', user_id);
//     fetchData('/mainpage/home/rating', 'ratingBasedVods', user_id);
//     fetchData('/mainpage/spotify', 'spotifyVods', user_id, 'post'); // POST 메서드로 Spotify VOD 데이터 가져오기
//   }, [fetchData]);
